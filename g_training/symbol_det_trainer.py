...
class SymbolDetectionTrainer:
    ...
    def train(self) -> Dict[str, Any]:
        ...
        # 多 GPU DataParallel
        # 注意: Faster R-CNN 的 DataParallel 需要特殊处理，
        # 因为它的 forward 接受 list[Tensor] 而非单个 Tensor。
        # 这里用自定义 wrapper 解决。
        gpu_ids = getattr(self, '_multi_gpu_ids', None)
        use_dp = gpu_ids and len(gpu_ids) > 1 and torch.cuda.is_available()
        if use_dp:
            logger.info("DataParallel: GPUs %s (Faster R-CNN 兼容模式)", gpu_ids)
